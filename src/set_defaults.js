...
export function setDefaults(t, lang, stateObj, c, o, sport, team, oppo) {
  ...
  if (stateObj.attributes.overunder) {
    c.pre2 = t.translate(
      sport + ".overUnder",
      "%s",
      String(stateObj.attributes.overunder)
    );
  }
  ...
  if (stateObj.attributes.team_win_probability) {
    c.barLength[team] = (
      stateObj.attributes.team_win_probability * 100
    ).toFixed(0);
  }
  c.barLength[oppo] = 0;
  if (stateObj.attributes.opponent_win_probability) {
    c.barLength[oppo] = (
      stateObj.attributes.opponent_win_probability * 100
    ).toFixed(0);
  }
  c.barLabel[team] = t.translate(
    sport + ".teamBarLabel",
    "%s",
    String(c.barLength[team])
  );
  c.barLabel[oppo] = t.translate(
    sport + ".oppoBarLabel",
    "%s",
    String(c.barLength[oppo])
  );
  ...
  if (stateObj.attributes.api_message) {
    ...
    var apiTail = stateObj.attributes.api_message.substring(
      stateObj.attributes.api_message.length - 17
    );
    if (apiTail.slice(-1) == "Z") {
      var lastDateForm = new Date(apiTail);
      c.notFoundTerm2 = t.translate(
        "common.no_upcoming_games",
        "%s",
        lastDateForm.toLocaleDateString(lang)
      );
    }
  }
